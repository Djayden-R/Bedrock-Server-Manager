...
def get_latest_release(repository_name: str, download_location: Path, file_name: Optional[str] = None):
    ...
    for asset in release["assets"]:
        ...
        if asset["name"] == file_name:
            file_save_path = Path(os.path.join(download_location, asset['name']))
            ...
            download_success = os.path.exists(file_save_path)

            if download_success:
                print("File is successfully downloaded")
            else:
                print(f"Problem during download of {repository_name}")

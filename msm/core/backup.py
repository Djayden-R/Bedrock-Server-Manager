...
def update_sym_link(cfg: Config, backup_path: Path):
    ...
    symlink = cfg.path_base / "latest_backup.zip"

    # Check if symlink already exists and remove it and the last_backup
    if symlink.exists():
        last_backup = symlink.resolve()
...
def quick_backup(cfg: Config):
    ...
    if not (cfg.backup_local_path or cfg.backup_hdd_path):
        return
    elif not cfg.backup_directories:
        raise ValueError("Please add the directories you want to backup to 'directories'")

    # Generate folder based on today's date and a file name based on the time
    backup_name, folder_name = generate_file_name()

    # Generate a zip file in main (path_base) folder
    backup_path = generate_zip(cfg, backup_name)

    if not backup_path:
        return

    for backup_location in (cfg.backup_local_path, cfg.backup_hdd_path):
        if backup_location:
            # Check if backup folders exists and create it if it doesn't
            ...
            # Move backup to location
            backup_folder = os.path.join(backup_location, folder_name)
            shutil.copy(backup_path, backup_folder)

    # Update last_backup symlink and remove the last backup
    update_sym_link(cfg, backup_path)  
